...
Run: `node "C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario\tests\storage.test.js"`
...
```bash
cd "C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario"
...
Run: `node "C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario\tests\importExport.test.js"`
...
window.PROJECTS_SEED = [
    { id: "quintoandar", label: "QuintoAndar" }
];
```
...
Run: `grep -n "window\." "C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario\documents.js"`
...
Run: open `C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario\index.html` directly in a browser (double-click it, or `start` it from a terminal).
...
Run: open (or reload) `C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario\index.html`.

Expected, in order:
1. Project select shows "QuintoAndar" already selected.
2. Results list shows the 2 migrated documents (Contrato de Locacao, Implementação W3).
3. Opening DevTools → Application → Local Storage shows keys `docscat_v1_projects`, `docscat_v1_active_project`, `docscat_v1_docs_quintoandar`.
...
6. Switch back to "QuintoAndar" via the select — confirm the "Teste" project's doc does not appear (isolation).
7. Click "Exportar JSON" on "QuintoAndar" — confirm a `quintoandar-export-<date>.json` file downloads with `project` + `documents`.
...
Run:
```bash
node "C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario\tests\storage.test.js" && node "C:\Users\GuilhermeRodriguesBa\Documents\SitesUteis\docs\glossario\tests\importExport.test.js"
```
...
```
Expected: after reload, "QuintoAndar" is the only project, selected by default, showing the 2 migrated documents — matching the spec's "Primeira carga" verification item.
