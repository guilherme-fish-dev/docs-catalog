...
1. Carrega a página → garante registro de projetos existe (seed de `projects.config.js` se primeira vez) → garante seed do projeto `quintoandar` existe (migração de `documents.js` se primeira vez) → renderiza com o último projeto selecionado (persistido também em localStorage, ex.: `docscat_v1_active_project`).
...
- `projects.config.js` — novo arquivo, seed inicial `[{id: "quintoandar", label: "QuintoAndar"}]`.
...
- Primeira carga: migração automática do projeto `quintoandar` com os 2 docs existentes.
